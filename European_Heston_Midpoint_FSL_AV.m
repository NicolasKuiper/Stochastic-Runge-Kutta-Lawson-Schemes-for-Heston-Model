...
%          Midpoint Full Stochastic Lawson scheme with anti-
%          thetic variance reduction technique to price 
%          European Options
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [type, option_price, std_deviation, elapsed_time] = European_Heston_Midpoint_FSL_AV(S0,r,V0,K,T,type,kappa,theta,sigma,rho,Nt,Nsim,R)
...
[~,~,X1] = MidpointFSLVectorized(X0,A,g0,B,g,tspan,h,dW1,g0Jac,gJac,Bexp);    % returns price at expiration, and price and volatility paths
X1 = real(X1);
% generate antithetic price at maturity
[~,~,X2] = MidpointFSLVectorized(X0,A,g0,B,g,tspan,h,dW2,g0Jac,gJac,Bexp);    % returns antithetic price at expiration, and price and volatility paths
X2 = real(X2);
% define option payoffs
if strcmp(type,'call')
    payoff = max(X1(1,:)-K,0);
...
antithetic_price = R*mean(antithetic_payoff);
antithetic_std = std(antithetic_payoff);
option_price = 0.5*(price + antithetic_price);
std_deviation = 0.5*sqrt(payoff_std^2+antithetic_std^2);
elapsed_time = toc;
